(function() {
  ...
  var Gmetric = (function(){
    ...
    Gmetric.prototype.unpack_string = function(buffer, pos){
      ...
      unpacked_string = buffer.toString('utf-8', pos, pos+strlen);
      pos += strlen;
    ...
    /**
     * Unpacks an xdr integer/boolean.
     * @param  {Buffer}  (buffer) The buffer to read from
     * @param  {Integer} (pos) The current buffer position
     * @return {Object}  The unpacked string and buffer position
     */
    ...
    Gmetric.prototype.parse_meta = function(meta_packet){
      var data = new Object(), unpack = null;
      ...
      unpack = this.unpack_string(data_packet, pos);
      pos += unpack.pos;
      data.spoof = unpack.string;

      // Parse metric type
      unpack = this.unpack_string(data_packet, pos);
      pos += unpack.pos;
      data.type = unpack.string;
      ...
      unpack = this.unpack_string(data_packet, pos);
      pos += unpack.pos;
      data.units = unpack.string;

      // Parse slope derivative
      unpack = this.unpack_int(data_packet, pos);
      pos += unpack.pos;
      data.slope = unpack.integer;

      // Parse tmax
      unpack = this.unpack_int(data_packet, pos);
      pos += unpack.pos;
      data.tmax = unpack.integer;

      // Parse dmax
      unpack = this.unpack_int(data_packet, pos);
      pos += unpack.pos;
      data.dmax = unpack.integer;
      ...
      for(var i = 0; i < extra_elems; i++) {
        ...
        unpack = this.unpack_string(buffer, pos);
        pos += unpack.pos;
        extra_key = unpack.string;
        unpack = this.unpack_string(buffer, pos);
        pos += unpack.pos;
        extra_value = unpack.string;
        data.extras[extra_key] = extra_value;
      }
